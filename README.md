...
- **🎉 최신 배포 (Save Point 105)**: https://e8812787.cswind-mto.pages.dev
- **개발 서버 (Sandbox)**: https://3000-i6ovkx4qstgf5tedcqtx9-a402f90a.sandbox.novita.ai

## ✅ 현재 완료된 기능 (Save Point 105 기준)

### 0. 프로젝트 간 도면 추적 (Cross-Project Traceability) ✅ (Save Point 105!)
- **사용자 요청**: 여러 프로젝트에서 같은 도면이 사용되고 있는지 추적하고 싶다
- **새로운 기능**:
  - ✅ **자동 도면 사용 감지**: BOM 테이블 렌더링 시 전체 프로젝트 스캔
  - ✅ **보라색 배지 표시**: Drawing 컬럼에 다른 프로젝트 사용 개수 표시 (예: 🔗 2)
  - ✅ **상세 정보 모달**: 배지 클릭 시 모든 사용 프로젝트 리스트 팝업
  - ✅ **프로젝트 정보 표시**: 프로젝트명, 고객사, 타입, 기간, BOM 아이템 개수
  - ✅ **도면 상태 구분**: 도면 있음/없음 상태 배지로 표시
  - ✅ **현재 프로젝트 강조**: 파란색 배경으로 현재 프로젝트 하이라이트
  - ✅ **빠른 프로젝트 전환**: "이동" 버튼으로 다른 프로젝트로 즉시 전환
  - ✅ **도면 없는 경우도 표시**: 현재 프로젝트에 도면이 없어도 다른 프로젝트에 있으면 배지 표시
- **사용 시나리오**:
  ```
  예시: E0004954053-00 도면이 3개 프로젝트에서 사용됨
  
  프로젝트 A (현재):
  - BOM 테이블 → Drawing 컬럼 → "📄 보기" + "🔗 3" 보라색 배지
  - 배지 클릭 → 모달 팝업
  - 프로젝트 A (현재 프로젝트) ✓ 도면 있음
  - 프로젝트 B (Vestas) ✓ 도면 있음 [이동] 버튼
  - 프로젝트 C (GE) ⚠️ 도면 없음 [이동] 버튼
  - "이동" 클릭 → 프로젝트 B로 전환
  ```
- **결과**:
  - 도면 재사용 현황을 한눈에 파악
